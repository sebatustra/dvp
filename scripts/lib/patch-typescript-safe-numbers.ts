...
/**
 * Hardens the generated TypeScript codecs against silent precision loss on
 * 64-bit arguments.
 *
 ...
 * DvP that is source-of-truth corruption (see `safeNumberCodecs.ts`): the
 * value is a trade amount or a PDA seed (the nonce).
 *
 * This patch, run after the JS render over both the instruction codecs and
 * the account codecs:
 ...
 *
 * Decoders (`getU64Decoder` / `getI64Decoder`) are left alone: they already
 * return `bigint` and never lose precision.
 *
...
export function patchTypeScriptSafeNumbers(typescriptClientsDir: string): void {
  ...
  // Both the instruction args and the account args serialize 64-bit fields.
  const dirs = [
    path.join(generatedDir, "instructions"),
    path.join(generatedDir, "accounts"),
  ];
  ...
  // Safety net: no unguarded 64-bit surface may survive in any codec.
  for (const dir of dirs) {
    for (const file of fs.readdirSync(dir)) {
      if (!file.endsWith(".ts")) continue;
  ...
  console.log(
    `Guarded 64-bit args against number rounding in ${filesPatched} TypeScript file(s).`,
  );
}
